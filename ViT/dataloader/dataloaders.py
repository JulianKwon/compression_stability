...
def mnist(batch_size=100, pm=False, resize=False):
    transf = [transforms.ToTensor()]
    if pm:
        transf.append(transforms.Lambda(lambda x: x.view(-1, 784)))
    if resize:
        transf.append(transforms.Resize(32))
...
def fmnist(batch_size=100, pm=False):
    transf = [transforms.ToTensor()]
    transf.append(transforms.Normalize((0.5,), (0.5,)))
    transform_data = transforms.Compose(transf)
...
def cifar10(augment=True, batch_size=128, resize=32):
