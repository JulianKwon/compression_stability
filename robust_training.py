...
def train(net, device, epoch, train_loader, criterion, optimizer, adversary):
    print('\n[ Train epoch: %d ]' % epoch)
    net.train()
    train_loss = 0
    correct = 0
...
def test(net, device, epoch, test_loader, criterion, file_name, adversary):
    print('\n[ Test epoch: %d ]' % epoch)
    net.eval()
    benign_loss = 0
    adv_loss = 0
    ...
    if not os.path.isdir('checkpoint'):
        os.mkdir('checkpoint')
    torch.save(state, '/workspace/paper_works/work_results/finally/' + file_name)
    print('Model Saved!')
...
if __name__ == '__main__':
    global learning_rate, epsilon, k, alpha, file_name

    learning_rate = 0.1
    epsilon = 0.0314
    k = 7
    alpha = 0.00784
    file_name = 'pgd_adversarial_training.pt'

    CUDA_NUM = 5
    device = 'cuda%d' % CUDA_NUM if torch.cuda.is_available() else 'cpu'
