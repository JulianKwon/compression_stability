...
def preTrain(name, EPOCH, device=6, lr=0.001, classes=10):
...
def sparse_coding(sparsities, EPOCH, device, name='resnet18', preTrained=False, classes=10):
    train_loader, val_loader, num_classes = imagenet(classes=classes)
    ...
    for sparsity in sparsities:
        ...
        if not preTrained:
            state = torch.load('runs/%s_%d/model_best.pth.tar' % (name, classes))
        ...
        for epoch in range(EPOCH):
            ...
            save_checkpoint(train_state, '%s_%d/sparsity%d' % (name, classes, sparsity), isBest,
                            filename='checkpoint_train.pth.tar')
            save_checkpoint(valid_state, '%s_%d/sparsity%d' % (name, classes, sparsity), isBest,
...
def inference(sparsity, name, model_name='resnet18', device=0, classes=10):
    train_loader, val_loader, num_classes = imagenet(classes=classes)
    criterion = nn.CrossEntropyLoss().to('cuda:%d' % device)
    ...
    #     model.load_state_dict(state['state_dict'])
    # else:
    state = torch.load('runs/%s_%d/sparsity%d/model_best.pth.tar' % (model_name, classes, sparsity))
    model.load_state_dict(state['state_dict'])
    model.to('cuda:%d' % device)
    ...
    save_data(train_state, "runs/%s/sparsity%d" % (name, sparsity), filename='train.pkl')
    save_data(valid_state, "runs/%s/sparsity%d" % (name, sparsity), filename='valid.pkl')
...
if __name__ == '__main__':
    ...
    if args.mode == 0:
        preTrain(args.resnet_mode, args.epochs, lr=args.lr, device=args.cuda_num, classes=args.num_class)
    elif args.mode == 1:
        sparse_coding(sparsity, args.epochs, name=args.resnet_mode, device=args.cuda_num, preTrained=args.pre_trained,
                      classes=args.num_class)
    elif args.mode == 2:
        for s in sparsity:
            inference(s, name="%s_sparsecoding_result" % args.resnet_mode, model_name=args.resnet_mode,
                      classes=args.num_class)
