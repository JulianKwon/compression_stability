...
parser.add_argument('--output_dir', type=str, default="/workspace/paper_works/work_results/ppogan",
                    help='GPUs to use.')
parser.add_argument('--snapshot_dir', type=str, default="/workspace/paper_works/work_results/ppogan",
                    help='snapshot')
parser.add_argument('--tb_dir', type=str, default="/workspace/paper_works/work_results/ppogan",
                    help='tensorboard')
...
CUDA_NUM = opt.cuda
device = torch.device("cuda:%d" % CUDA_NUM)
...
def calc_gradient_penalty(netD, real_data, fake_data, BATCH_SIZE):
    ...
    alpha = alpha.expand(BATCH_SIZE, real_data.nelement() // BATCH_SIZE).contiguous().view(BATCH_SIZE, 3, ImageSize,
                                                                                           ImageSize)
...
schedulerD = optim.lr_scheduler.LambdaLR(optimizerD, lr_lambda=[
    lambda epoch: max(0., 1. - epoch / (CONF['niter'] * len(dataloader)))])
optimizerD_D = optim.Adam(D.parameters(), lr=CONF['LR'] / CONF['PPO_iters'], betas=(0.0, 0.9))
schedulerD_D = optim.lr_scheduler.LambdaLR(optimizerD_D, lr_lambda=[
    lambda epoch: max(0., 1. - epoch / (CONF['niter'] * len(dataloader)))])
optimizerG = optim.Adam(netG.parameters(), lr=CONF['LR'] / CONF['PPO_iters'], betas=(0.0, 0.9))
schedulerG = optim.lr_scheduler.LambdaLR(optimizerG, lr_lambda=[
    lambda epoch: max(0., 1. - epoch / (CONF['niter'] * len(dataloader)))])
...
while G_progress < niter * len(dataloader):
    ...
    while i < len(dataloader):
        ...
        while j < CONF['Diters'] and i < len(dataloader):
            ...
            # print(mone.data)
            errD_real.backward(mone[0], retain_graph=True)
            ...
            if j % 50 == 0:
                print('Progress[{0}] Iter[{1}/{2}]\tError D fake: {errD_fake:.3f}\tError D real: {errD_real:.3f}'.format(
                    G_progress, j, CONF['Diters'], errD_fake=errD_fake, errD_real=errD_real
                ))
        ...
        while j < CONF['PPO_iters'] and i < len(dataloader):
            ...
            output = D(real_tmp)
            output, label = output.type(torch.FloatTensor), label.type(torch.FloatTensor)
            errDD_real = criterion(output.squeeze(), label.squeeze())
            errDD_real.backward()
            ...
            output, label = output.type(torch.FloatTensor), label.type(torch.FloatTensor)
            errDD_fake = criterion(output.squeeze(), label.squeeze())
            errDD_fake.backward()
            ...
            errG = target.mean()
            errG.backward(mone[0])
            D_G_z2 = errG.item()
            optimizerG.step()

            if j % 50 == 0:
                print(
                    'Progress[{0}] Iter[{1}/{2}]\tError D real: {errDD_real:.3f}\tError D fake: {errDD_fake:.3f}\tError Generator: {errG:.3f}'.format(
                        G_progress, j, CONF['Diters'], errDD_real=errDD_real, errDD_fake=errDD_fake, errG=errG
                    ))
        ...
        schedulerG.step()
        schedulerD_D.step()
    # writer.add_image("real_sample", torchvision.utils.make_grid(real_cpu, nrow=8, normalize=True),
    #                  global_step=G_progress)
    # writer.add_image("fake_sample", torchvision.utils.make_grid(fake.data, nrow=8, normalize=True),
    #                  global_step=G_progress)
print(progress="{}/{}".format(niter * len(dataloader), niter * len(dataloader)),
      caption="Done... Best IS:{}".format(bIS))
